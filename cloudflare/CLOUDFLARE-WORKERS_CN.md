...
2. 【请求级配置隔离】实现了基于每次请求独立创建配置副本的机制。
 ...
 10. 【自动代理池与24小时更新】
     ...
     - best-of-2 评分选路（默认轮询模式）：最快代理获得最高选中概率，慢速代理仍保留一定流量以便持续验证可用性
     ...
     - 自动故障转移：单代理请求失败自动轮换下一个，所有代理失效时自动回退直连，确保服务高可用      - 单代理测试含真实 TLS 握手（证书校验），伪造 gemini.google.com 证书的 TLS-MITM 代理会被直接拒绝
      - 单代理测试默认超时（隧道 + TLS 握手）2000ms（可通过 PROXY_TEST_TIMEOUT_MS 调整）
